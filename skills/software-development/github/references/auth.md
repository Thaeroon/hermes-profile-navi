...
>
> ```bash
> # $TOKEN=*** access token from the device flow above (never echo it)
> mkdir -p ~/.config/gh
...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  export GITHUB_TOKEN=*** "^GITHUB_TOKEN=*** "$_hermes_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=***
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=*** run python "${HERMES_HOME:-$HOME/.hermes}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=***
else
  echo "AUTH_METHOD=***
